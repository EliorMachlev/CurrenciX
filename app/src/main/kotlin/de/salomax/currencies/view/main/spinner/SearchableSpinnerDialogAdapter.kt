...
class SearchableSpinnerDialogAdapter(
    ...
    override fun getItemViewType(position: Int): Int = if (position == itemCount - 1) VIEW_TYPE_API_HINT else VIEW_TYPE_RATE
    ...
    override fun onBindViewHolder(
        ...
        holder.btnStar.setImageDrawable(
            if (stars.contains(item.currency)) {
                drawableFav
            } else {
                drawableFavEmpty
            },
    ...
    fun isDraggable(position: Int): Boolean = filterText.isNullOrEmpty() && isStarred(position)
    ...
    private fun buildConversionText(item: Rate): String {
        ...
        val left =
            if (sourceSymbol.isEmpty()) {
                source
            } else if (hasAppendedCurrencySymbol(context)) {
                "$source $sourceSymbol"
            } else {
                "$sourceSymbol $source"
            }
        val right =
            if (destinationSymbol.isEmpty()) {
                destination
            } else if (hasAppendedCurrencySymbol(context)) {
    ...
    private fun update() {
        val filtered =
            rates
                // Match rates against `filterText` on either the currency's full name
                // or its ISO 4217 code, so a search like "usd" or "dollar" both work.
                .filter { rate ->
                    if (filterText != null) {
    ...
    inner class ViewHolder(
        ...
        private fun findOriginalPosition(filteredPosition: Int): Int = rates.indexOf(ratesFiltered[filteredPosition])
    }
}
